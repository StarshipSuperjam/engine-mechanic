...
V1 deliberately makes one repository the durable authority for one project's state, memory, decisions,
guardrails, evidence, and product work. That boundary produces the qualities the Engine is designed around:
...
| --- | --- | --- |
| Repository topology and versioned template | A project repository is the durable source of truth | Every project remains sovereign; fleet membership is an explicit, reversible relationship |
...
| Memory, knowledge, and state separation | Raw work and durable truth have different homes | Only bounded receipts and reviewed artifacts cross the project boundary; raw session memory stays local |
...
1. **The project repository remains authoritative.** The company layer may observe, propose, and broker; it
   cannot silently revise project truth.
...
3. **The company service is never the sole copy of project truth.** A project can reconstruct its approved
   posture from its repository and local Engine state.
...
11. **Brokered means non-bypassable.** A provider family is called brokered only when credential custody and,
    where feasible, provider or network controls prevent direct unrecorded use. Any successful bypass fails
    adapter admission. The cockpit may label that candidate integration as partial, but the company layer must
    not call it brokered or make a complete-action claim.
...
after policy classifies the action and that adapter proves custody, confinement, observation, and recovery.
...
flowchart TB
    ...
    AB["Provider-specific action adapter<br/>v1 exercise gate · receipts"]
    CR["Credential custody<br/>provider limits"]
    ...
    AP -- "protected-action approval" --> AB
    CR -- "short-lived provider capability" --> AB
    AB -- "exact admitted action" --> EX
    EX -- "outcome" --> AB
...
versions, offline behavior, and removal procedure. Enrollment adds a company project identifier and a
non-copyable instance key proved by possession; the private key is local state, never repository content.
Cloning, reinstalling, rotating, revoking, and re-enrolling create distinct instance identities and receipts,
so a copied manifest cannot impersonate its source. Joining is a reviewed project change; departure is a
...
requires a new successful adoption receipt. Thus sovereignty controls what enters the repository, while the
...
The company layer publishes signed, immutable versions of policy packs and optional capability packs. Company
packs are organization-authored modules, not a second packaging system. Their ids and owned paths live in an
organization namespace; every file still has one provider, wiring uses the v1 closed vocabulary, and the
manifest declares compatible Engine releases, dependencies, ownership, migration, rollback, and removal.
Installation refuses module-id, file-provider, wiring, or version collisions. Company pack owners cannot
claim Engine-owned paths or bypass the project's normal ownership and protected-review treatment.
...
exercise. A mismatch or stale attestation fails closed. Projects and runtimes receive a narrow capability,
never the long-lived credential. Any authority in the chain can narrow or revoke; no authority can enlarge an
...
spend limits, idempotency, independent provider-side observation, reconciliation, recovery, and conformance
fixtures. V1 deployment adapters are the first candidate. Sending, publishing, spending, deleting durable
...
adapter verifies the complete v1 chain and the company constraint. Deletion, spending, external publication
or communication, privilege escalation, and actions without reliable reversal are non-waivable protected
classes: an independent human approval bound to the complete request digest is always required. Approval has
...
| Policy adoption | `current`, `pending`, `stale`, `conflicted`, `waived` |
| Health | `healthy`, `degraded`, `unknown` |
| Company authority | `eligible`, `restricted`, `suspended` |

Each axis has independent transition, receipt, expiry, and recovery rules. A project can therefore be online,
current, degraded, and eligible at the same time without losing information.
...
rewrite the other's records. Exit revokes outstanding grants, stops new receipt emission, removes connector
wiring, retains the local record of adopted policy and action receipts, and identifies any company pack whose
removal needs an explicit migration. The product remains runnable and the local Engine remains capable of
cold start.

The company layer itself can be replaced. Export formats cover policy-pack history, fleet membership,
waivers, promoted artifacts, and broker receipts without exporting private project memory that was never in
scope.
...
whether a small cohort or linked fields can reconstruct project-private facts.
...
sequenceDiagram
    ...
    B->>C: Exact admitted provider capability request
    C-->>B: Short-lived provider capability within provider limits
    B->>X: Exercise exact admitted action
    X-->>B: Outcome
    X-->>B: Independent observation or reconciliation anchor
...
| Approval or credential-custody compromise | The remaining independent domains fail closed, provider limits bound impact, and provider-side reconciliation identifies effects |
...
| --- | --- | --- | --- |
| A. Federate | Enrollment, instance identity, bootstrap collection policy, receipt transport, recovery roots | Repository topology, control plane, validators, runtime adapters | Clone resistance, reconnect, backpressure, replay, key rotation, backup restore, and private-data exclusion pass |
| B. Observe | Fleet registry, bounded signed receipts, exception cockpit | Stage A; telemetry, delivery evidence, operator cockpit, evidence explorer | A multi-project operator finds health, evidence, drift, and pending-decision exceptions with complete provenance |
| C. Govern | Namespaced signed packs, adoption proposals, precedence, waivers, deadline consequences | Stage A; control plane, modules, provisioning, validators | A rule can be adopted, rejected, strengthened, waived, suspended, restored, rolled back, and audited without silent mutation |
| D. Authorize | Organizational constraints composed with the v1 authority chain | Stage A; authority-broker contract, credential broker, runtime adapters | Identity consistency, separation of duties, revocation, expiry, local narrowing, and effective-policy mismatch pass |
| E. Act | One admitted provider-family adapter, beginning with deployment if it qualifies | Stages C-D; deployment, operations, bounded repair, delivery evidence | Direct bypass fails, protected actions require human approval, provider observation reconciles, and recovery works |
| F. Learn | Cross-project evals and reviewed artifact promotion | Stages A-B; product knowledge graph, research-and-learning, profile registry | Multi-project evidence beats an anecdote; false evidence is quarantined; promotion remains advisory and reversible |
...
| Removing the company integration leaves the product runnable | Perform the documented exit flow and run the project's ordinary product and Engine smoke checks | operator |

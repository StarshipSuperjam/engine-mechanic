...
| A hook failure is a finding, promoted immediately: a gate that cannot evaluate is an enforcement failure, never a silent pass. | Derived from this document's own normative statement; how it is verified is defined when this capability is settled. | operator |

...
| --- | --- | --- |
| **One cursor, schema-validated.** A single machine-state file governed by the [schemas](../surfaces/schemas.md) surface; the malformed file fails loud (above), so a corrupt cursor never misleads. | Not recorded in the design workspace — how this is verified is defined when this capability is settled. | operator |

...
*Heritage (editorial, not a criterion): attention is the prioritization piece of context engineering — the discipline of deciding what to load, exclude, prioritize, and prune in the context window, which the orientation family as a whole instantiates; in CoALA terms it sits within the decision-making procedure. (See the glossary Lineage cluster — maintainer vocabulary only, never operator-facing.) Moved out of the criteria table at settling: an attestation nothing could falsify is not a criterion ([decision 0331](../../../adr/0331-settle-the-reconciled-corpus-as-the-build-baseline.md)).*

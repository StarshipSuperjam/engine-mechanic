...
Canonical skill frontmatter contributes explicit **`routes-to`** edges. A route names an operation, tool,
or subordinate skill in structured `engine-targets` metadata; the generator records that target and any
conditional module availability. It does not infer delegation from prose. The graph therefore explains
both what a route enters and whether it is active, conditional, or home-only to the generated self-map,
without becoming an authority source of its own.
...
| **Route targets are explicit.** Skill-to-operation/tool/sub-skill routing and conditional availability come from structured frontmatter, never a prose guess. | Route-generation tests require a target for every automatic route and require each target to exist or be explicitly conditional/home-only; regenerated graph bytes are fingerprint-gated. | engine |
...
| **A swappable representation/retrieval leaf** — the graph's representation is bound via the [interface](../surfaces/interfaces.md) surface so a richer engine can slot in (and be bounded) without reopening knowledge; the graph stays *derived* (above), not hand-authored. At scale a dense-graph representation risks hub-explosion ([R8](../../../reference/risks.md)) — which **stays open**, *deferred* behind the swappable seam rather than *mitigated* (keeping the representation swappable is the ability to change it later, not a present containment of hub-explosion); candidate engines are tracked in [open-questions](../../../reference/open-questions.md), not named here. | The swappable-leaf leg is realized by the declared retrieval interface, with the interface-declaration checks as partial support. The deferred-not-mitigated stance is design narrative no check can assert — your read. | operator |
...
*Heritage (editorial, not a criterion): CoALA semantic memory — but only its structure. Where CoALA-semantic is belief-bearing, belief lives in [memory](memory.md) per the wall above; CoALA leaves representation open (unstructured text to structured stores), which is what licenses the swappable leaf. (See the glossary Lineage cluster — maintainer vocabulary only.) Moved out of the criteria table at settling: a lineage attestation nothing could falsify is not a criterion ([decision 0331](../../../adr/0331-settle-the-reconciled-corpus-as-the-build-baseline.md)).*

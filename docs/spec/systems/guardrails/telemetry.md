...
| --- | --- | --- |
| **Self-surfacing, not self-healing** — triage (open/update an issue) is the only autonomous step; the AI remediates next session, under guardrails, and the operator merges. Never claim it heals unattended. | Observe the tool's runnable demo walkthrough: triage's only writes are issue open/update/close, and the degraded path makes none. The unit test asserting zero issue writes on a degraded read is partial support; the demo's aggregate self-check is inert at the pin (a defect swallowed by its fail-open handler — it cannot fail the run), so it supports nothing. No check asserts the whole never-heals claim. | operator |

...
*Settled in the design workspace on 2026-05-31, ratified by [decision 0156](../../../adr/0156-name-the-engine-s-execution-substrate-a-group-scoped-uv-mana.md).*
...
| --- | --- | --- |
| **Code, not prose or persona** — `tool` executes deterministically; reading-and-following procedures are operations, isolated-context work is agents. | Read this description against the built behavior and confirm they match. | operator |
| **No schema, no template** — the sanctioned empty case for class `code`; governed by tests and checks. | The design states this is enforced mechanically; the mechanism is named in the criterion. | engine |
| **The code-home for the substrate** — validators, hooks, MCP servers, the wiring library, and interface implementations, all under `.engine/tools/`. | Read this description against the built behavior and confirm they match. | operator |
| **Python, in the tool-runtime** — `tool` code is Python, executed through the engine's uv-managed [tool-runtime](../infrastructure/repository-topology.md) (a pinned interpreter, isolated from the operator's system Python); a surface-realization fact, the `class: code` meta-contract record unchanged ([D-156](../../../adr/0156-name-the-engine-s-execution-substrate-a-group-scoped-uv-mana.md)). | Read this description against the built behavior and confirm they match. | operator |

...
Launch reauthorizes every product and Engine subject against the bundle's recorded scope. Authorization
revocation or narrowing deactivates every affected pointer and triggers verified purge of the system-managed
bundle, receipt, indexes, and derived records; regeneration may restore only the newly permitted scope. A
direct-open file cannot reauthorize its reader and says so inside the static content. Keeping or copying a
bundle outside its managed storage is an unsupported export, not a way to preserve access after revocation;
the Engine makes no claim that it can erase an uncontrolled copy or what an already-authorized person learned.
...
derived bundle, cache, index, authoring proposal, progress record, diagnostic, activation receipt, and pointer
for a subject, and verifies that no current pointer, protected receipt, or retention record still names its
subject, authorization scope, manifest, or generation. Fixing the source is not considered sufficient deletion.
...
| **Resolution preserves context in both directions** — the operator can move Landscape → System → Integration → Implementation → Evidence and back without losing selection, subject, revision, or return context. | On both an ordinary product and `engine-mechanic`, an operator follows one seeded integration through every resolution and returns to the original landscape selection; the scenario fails on a changed subject, revision, selection, broken deep link, or lost return path. | operator |
...
| **Degraded guidance leads somewhere safe** — an operator can understand the limitation, its consequence, and an available recovery or escalation path without being stranded. | Operator scenarios separately exercise unsupported code, stale evidence, corrupt bundles, and denied authorization; completion requires identifying what remains trustworthy and successfully taking or explaining the stated safe next step. | operator |
...
| **Authorization is non-amplifying and purge is complete** — product and Engine access are checked independently and system-managed derived data does not outlive deletion or authorization. | Separate-principal and authorization-revocation/narrowing fixtures prove depth controls and direct-open paths do not bypass source permissions; purge inventories and removes bundles, indexes, proposals, progress, diagnostics, activation receipts, pointers, and retention records, then proves none still names the subject, authorization scope, manifest, or generation. | engine |

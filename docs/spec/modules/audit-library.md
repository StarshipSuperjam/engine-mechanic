...
| **required core, not one of the eleven foundations** — deployed-repo self-audit hygiene ships in every generated repo and is not an install choice ([D-067](../../adr/0067-operator-facing-module-packaging-industry-discipline-categor.md)). | Operator observation: the module manifest declares `distribution: required` and the module is absent from the extension-add catalog. The module-manifest check (hard, CI) asserts only that the manifest is schema-valid — it does not pin the distribution to `required` — so the always-present claim is your read of the manifest. | operator |

...
| **Validation dispatcher + closed kinds in core, rule corpus in `validators-core`** — forced by the locked kind-registry shape and the `core → validators-core` dependency direction, not a style choice. | Operator observation: the dispatcher's hardcoded registry (the closed five plus the `custom/script` escape hatch) lives in core's code-home while the rule corpus files ride the validators-core manifest. Partial support: hard-check-bite (hard, CI) proves the kinds ship and bite from core's registry, and the census checks tie each rule file to its declaring module — neither asserts the partition itself. | operator |

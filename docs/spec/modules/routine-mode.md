...
| **`required` core, not a foundation** — the routine stance ships in every generated repo and is never an install choice ([D-067](../../adr/0067-operator-facing-module-packaging-industry-discipline-categor.md)); the eleven foundations stay minimal beneath it. | Operator observation: the manifest declares `distribution: required`, the self-map renders it required, and the module is absent from the extension-add catalog so it is never offered as a choice. Partial support: module-manifest (hard, CI) holds the status field schema-valid without pinning its value; provisioning-catalog (hard, CI) governs the catalog it is correctly missing from. | operator |

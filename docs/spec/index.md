...
This describes **engine-template** — the product this repository builds — as it is **actually built**: every row marked **settled** carries that promise, and the rows marked otherwise are the disclosed exceptions. First written in a separate design workspace, carried here as intended design, and reconciled document by document against the build, the corpus is now **settled**: on 2026-08-02 the operator accepted every reconciled capability document as the ratified baseline ([decision 0331](../adr/0331-settle-the-reconciled-corpus-as-the-build-baseline.md)) — the ground future builds are measured against and future design work diffs from. One stub remains **not yet described** (engine-knowledge-graph). Most **in progress** rows are forward-designed documents describing intended work not yet built, authored after the settling through the plan-acceptance route ([decision 0327](../adr/0327-route-product-spec-authoring-through-plan-acceptance-into-b.md)): [platform-currency](modules/platform-currency.md), and the twenty-six documents of the **delivery-plane program** ([decision 0334](../adr/0334-adopt-the-delivery-plane-spec-program-module-map-wave-order.md)) — each entering as a draft, settling per wave by the operator's recorded acceptance before that wave's build begins. One additional detailed draft, the [product-system explorer](modules/product-system-explorer.md), is preserved but parked outside the build order by [decision 0337](../adr/0337-preserve-the-product-system-explorer-design-for-late-v1.md). The former `clean-code` stub is **retired** by decision 0334: the engineering-quality family absorbs its territory, with the decision record and git history as the trace.
...
The **in-progress** [product-system explorer](modules/product-system-explorer.md) is fully described so its
research does not have to be reconstructed, but remains explicitly parked: not settled, scheduled, or present
in the build order. Its mandatory late-v1 revalidation decides whether it joins or follows delivery wave 7,
remains parked, changes shape, or is retired; only that later recorded decision can make it build ground. Its
intended future replacement of `engine-parts` is conditional on a separate parity, compatibility, and
supersession decision; the current command and core contract remain unchanged.

...
| Project memory | The committed memory substrate (ledger, MCP recall, pins, curation) | Native auto-memory | Engine store is authoritative; native notebook actively fenced (writes denied, declared non-citable). Recommended to stand as-is (ratification rides the migration decision record). |
...
| Instruction dedup across runtimes | Two hand-maintained floors + parity tooling | @AGENTS.md import / symlink | Deliberately declined: runtime-specific wording wins. Recommended to stand as-is (ratification rides the migration decision record). |

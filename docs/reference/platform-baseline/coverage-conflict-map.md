...
| Project memory | The committed memory substrate (ledger, MCP recall, pins, curation) | Native auto-memory | Engine store is authoritative; native notebook actively fenced (writes denied, declared non-citable). Ratified. |
...
| Packaging/distribution | module_manager + instantiator + release workflows | Plugins + marketplaces | Engine packaging ratified; revisit only if marketplace distribution becomes a goal. |
| Instruction dedup across runtimes | Two hand-maintained floors + parity tooling | @AGENTS.md import / symlink | Deliberately declined: runtime-specific wording wins. Ratified. |

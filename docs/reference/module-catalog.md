...
> Under the three-axis grammar ([D-335](../adr/0335-separate-module-distribution-applicability-and-activation.md)),
> **distribution** decides what the install enumerates: only `extension` modules are declinable, so the
> install walkthrough presents just those, grouped under three recognized SDLC discipline categories
> ([D-067](../adr/0067-operator-facing-module-packaging-industry-discipline-categor.md)'s category model, which
> stands). The **required spine** — `core` plus the governance modules that D-335 promoted from optional — is
> never an install choice. Decision 0068's Q1 *cut* roster stands; what D-335 supersedes is only the optional
> *classification* of the five promoted governance modules.
...
### Required-distribution packages — never an install choice

These ship and upgrade with every Engine. Per [D-067](../adr/0067-operator-facing-module-packaging-industry-discipline-categor.md) /
[D-335](../adr/0335-separate-module-distribution-applicability-and-activation.md) they are not enumerated in the
install walkthrough and are not individually removable; they are disclosed in plain language in the project
README. Required distribution is **not** always-active: the governance modules below are present in every
Engine but activate only when their subject matter applies (their [module docs](../spec/index.md) carry the
applicability/activation detail).

| Module | Distribution | Depends on |
|---|---|---|
...
| [product-design](../spec/modules/product-design.md) | required (promoted, D-335) | core |
| [dependency-discipline](../spec/modules/dependency-discipline.md) | required (promoted, D-335) | core |
| [migration-discipline](../spec/modules/migration-discipline.md) | required (promoted, D-335) | core |
| [design-review](../spec/modules/design-review.md) | required (promoted, D-335) | core |
| [qa-review](../spec/modules/qa-review.md) | required (promoted, D-335) | core |
...
The `extension` modules the install walkthrough presents — the only distribution class an operator declines
([D-335](../adr/0335-separate-module-distribution-applicability-and-activation.md)): three entries, one offered
opt-out (on by default) and two opt-in. Grouped under recognized SDLC discipline categories
([D-067](../adr/0067-operator-facing-module-packaging-industry-discipline-categor.md); the grouping is provisioning
selection-UX presentation data keyed by module `id`, not a manifest field). *Offered-on-vs-opt-in is
provisioning presentation data, not a manifest axis.*

| Category | Module | Distribution | At setup | Depends on |
|---|---|---|---|---|
| Product Management | [github-projects-sync](../spec/modules/github-projects-sync.md) | extension | opt-in | core |
| Software Configuration Management | [external-contribution](../spec/modules/external-contribution.md) | extension | opt-in | core |
| Software Configuration Management | [memory-semantic-recall](../spec/modules/memory-semantic-recall.md) | extension | offered on (opt-out) | core, memory-substrate-sqlite-fts5 |

The five governance modules that once sat on this menu (`product-design`, `dependency-discipline`,
`migration-discipline`, `design-review`, `qa-review`) are now required distribution and appear in the
required-spine table above; they are no longer declinable at setup.
...
### Future / post-v1 slots — named, not yet built (not on the operator menu)

Slots held in the end-state catalog whose design is deferred to a dedicated session and whose build is **out of v1 scope**. These are named here so the grammar has a home, not hidden; their distribution class is decided at their design session, not here. The **Standing** column below is document/lifecycle stage (retired · stub · in progress) — not a distribution class, which these slots do not yet carry.

| Module | Standing | Depends on |
|---|---|---|
| clean-code | retired ([decision 0334](../adr/0334-adopt-the-delivery-plane-spec-program-module-map-wave-order.md)) — territory absorbed by [engineering-quality](../spec/modules/engineering-quality.md) | — |
| [engine-knowledge-graph](../spec/modules/engine-knowledge-graph.md) | stub (post-v1) | *not settled — see its note below* |
| [product-knowledge-graph](../spec/modules/product-knowledge-graph.md) | in progress — full forward design under the delivery-plane program ([decision 0334](../adr/0334-adopt-the-delivery-plane-spec-program-module-map-wave-order.md)) | core, delivery-core, code-intelligence-core |

**`clean-code`** — **retired** by [decision 0334](../adr/0334-adopt-the-delivery-plane-spec-program-module-map-wave-order.md): the [engineering-quality](../spec/modules/engineering-quality.md) family (the stack-declared quality contract and its per-stack profiles) absorbs the code-style/lint territory this stub reserved under [D-095](../adr/0095-cut-expression-contracts-disposition-prose-organization-cove.md), broadening it from style conformance to the full quality surface (formatter, linter, types, build, tests, dependency checks) so that “style clean” can never stand in for “software correct.” The v1 no-lint-floor disclosure now names engineering-quality as the planned remedy; the stub's design threads live on in that family's documents, and the git history holds the stub's text.

**`engine-knowledge-graph`** ([D-086](../adr/0086-cognitive-foundations-as-required-packages-reconciliation-me.md)) — the deferred **graph-representation layer** over the engine's memory ledger, a capability the build keeps distinct from the shipped find-by-meaning module (which is a vector store answering recall questions, deliberately *not* a knowledge graph — [memory-semantic-recall](../spec/modules/memory-semantic-recall.md)). Not yet built; its stub records that even its catalogued dependency edge is **not settled** — re-derived at its design session under the target-axis discriminator ([D-129](../adr/0129-reconcile-dependency-discipline-to-depends-core-l2-the-targe.md)), where its representation/retrieval engine choice ([Q11](open-questions.md)) also lands. It is engine-infra, not an operator discipline, so its menu placement is likewise a promotion-time call.
...
Scope note (operator-facing packaging, [D-067](../adr/0067-operator-facing-module-packaging-industry-discipline-categor.md) / [D-335](../adr/0335-separate-module-distribution-applicability-and-activation.md)): the install walkthrough enumerates
only the declinable packages above — the three `extension` modules — grouped under the **Product Management /
Software Configuration Management / Verification & Validation** discipline categories. The required spine
(now including `product-design`, `dependency-discipline`, `migration-discipline`, `design-review`, `qa-review`
alongside `routine-mode` and `audit-library`) is never presented as an install choice, and
...
expressible as check rules / a future V&V lens). Two were **kept** as Software Configuration
Management modules: `dependency-discipline` and `migration-discipline` — now **required** distribution
([D-335](../adr/0335-separate-module-distribution-applicability-and-activation.md)), though still activating
only when the product has dependency/migration changes to govern.
...
Scope note (cognitive substrate, [D-086](../adr/0086-cognitive-foundations-as-required-packages-reconciliation-me.md)): the cognitive floor is part of the required spine. The [memory](../spec/systems/cognitive/memory.md) foundation floor is its own `required` package (`memory-substrate-sqlite-fts5`) — it owns the engine's only gitignored, non-regenerable per-instance store (the NDJSON ledger, so its schema needs an owned migration unit) and the server seam the semantic layer binds to. The [knowledge](../spec/systems/cognitive/knowledge.md) foundation has no separate package: its committed entities + derived index ride `core`'s `provides` (regenerable, no per-instance store to migrate). **Semantic recall** is the shipped [memory-semantic-recall](../spec/modules/memory-semantic-recall.md) `extension` (offered on at setup) atop the ledger — the [D-024](../adr/0024-the-engine-is-upgradeable-versioned-packages-upgraded-by-ove.md) split of FTS5-floor (required) from semantic-module (a declinable extension), now realized in the catalog rows and dependency graph above; the deferred **graph representation** question stays with the [engine-knowledge-graph](../spec/modules/engine-knowledge-graph.md) stub ([Q11](open-questions.md)).
